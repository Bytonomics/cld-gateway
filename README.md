...
| No | Config                                                               | Value / example                                                                                                | Behavior                                                                                        | How to choose                                                                        |
|----|----------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|
| 1  | `workflow.context_management.enabled`                                | Purpose                                                                                                        | Master switch for Gateway-side context pruning before translation/forwarding.                   | Keep enabled for long Claude Code sessions unless debugging raw history.             |
|    |                                                                      | `true`                                                                                                         | Gateway may apply request edits, configured edits, and hard limits.                             | Default; use for normal operation.                                                   |
...
|    |                                                                      | `follow_request`                                                                                               | If Claude Code sends `context_management.edits`, use those; otherwise use `default_edits`.      | Default; best match for Anthropic-compatible behavior with Gateway fallback policy.  |
|    |                                                                      | `override_request`                                                                                             | Ignore Claude Code request edits and use `override_edits`.                                      | Use when operators need a hard Gateway-owned pruning policy.                         |
...
| 3  | `workflow.context_management.default_edits`                          | Purpose                                                                                                        | Context edit list used only when `mode: follow_request` and Claude Code sends no request edits. | Use for a Gateway fallback policy without overriding Claude Code.                    |
...
| 4  | `workflow.context_management.override_edits`                         | Purpose                                                                                                        | Replacement edit list used only when `mode: override_request`.                                  | Use when Gateway policy must override Claude Code’s requested policy.                |
|    |                                                                      | `null` or omitted                                                                                              | No override edits; only hard limits can prune when `mode: override_request`.                    | Default; do not use `override_request` with this unless hard limits are enough.      |
...
|    |                                                                      | unknown prefix                                                                                                 | Edit is ignored and reported in context-management metadata.                                    | Avoid; it will not prune anything.                                                   |
| 6  | `workflow.context_management.*_edits[].trigger.type`                 | Purpose                                                                                                        | Optional condition that decides whether a `clear_tool_uses*` edit activates.                    | Omit the trigger to always activate that edit.                                       |
|    |                                                                      | `tool_uses` with `value: 20`                                                                                   | Activates only when the request has more than 20 tool interactions.                             | Use for tool-heavy agent sessions.                                                   |
|    |                                                                      | `input_tokens` with `value: 50000`                                                                             | Activates only when estimated request tokens exceed 50k.                                        | Use when pruning should be based on approximate prompt size, not raw tool count.     |
...
|    |                                                                      | omitted for `clear_thinking*`                                                                                  | Keeps 1 thinking turn.                                                                          | Use when you want the default thinking-prune behavior.                               |
| 8  | `workflow.context_management.*_edits[].clear_at_least`               | Purpose                                                                                                        | Optional minimum estimated token savings required before clearing tool history.                 | Use to avoid pruning when the savings would be too small to matter.                  |
...
| 9  | `workflow.context_management.*_edits[].exclude_tools`                | Purpose                                                                                                        | Tool names that should not be cleared by `clear_tool_uses*`.                                    | Use for tools whose output must remain visible for correctness.                      |
|    |                                                                      | `[]`                                                                                                           | All tool interactions are eligible for clearing.                                                | Default; simplest policy.                                                            |
...
| 11 | `workflow.context_management.hard_limits.max_tool_result_chars`      | Purpose                                                                                                        | Clears any individual tool result whose serialized content exceeds this character limit.        | Hard limit runs after normal context edits.                                          |
...
| 12 | `workflow.context_management.hard_limits.max_tool_uses_to_keep`      | Purpose                                                                                                        | Hard cap on retained tool-use/tool-result interactions after normal context edits run.          | Use as an operator safety rail.                                                      |
...
| 13 | `workflow.context_management.hard_limits.max_thinking_turns_to_keep` | Purpose                                                                                                        | Hard cap on retained assistant thinking/redacted-thinking turns after normal context edits run. | Use when old thinking keeps steering new tasks.                                      |
...
| No | Config                                        | Value / example  | Behavior                                                                                                                                                                                                                                                                                                  | How to choose                                                                                                                                 |
|----|-----------------------------------------------|------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------|
| 1  | `workflow.claude_code.slash_commands.enabled` | Purpose          | Controls whether Gateway parses Claude Code’s expanded slash-command envelope as structured instructions/input.                                                                                                                                                                                           | Keep enabled so `/commit`, `/review_agent`, `/make-tasks-for-plan`, and similar commands follow their expanded instructions.                  |
|    |                                               | `true`           | Enables slash-command translation. The selected mode decides how the command is translated.                                                                                                                                                                                                               | Default; normal gateway behavior.                                                                                                             |
|    |                                               | `false`          | Disables slash-command translation and forwards command tags as ordinary text.                                                                                                                                                                                                                            | Use only for debugging raw Claude Code requests; OpenAI may focus on the command name and ignore the expanded body.                           |
| 2  | `workflow.claude_code.slash_commands.mode`    | Purpose          | Selects which slash-command envelope is treated as active and how it is translated.                                                                                                                                                                                                                       | This only matters when `slash_commands.enabled: true`.                                                                                        |
|    |                                               | `promote_latest` | Only supported value. If the latest user message contains `<command-name>` / `<command-args>`, Gateway moves the expanded command body into backend `instructions`, sends only command args as user input, strips command tags from user input, and compacts older slash-command envelopes as historical. | Use this for Claude Code custom slash commands; it mirrors Codex-style slash dispatch where command instructions and user args are separated. |
...
| 3  | `workflow.claude_code.skills.enabled`         | Purpose          | Controls whether explicitly expanded Claude Code skill instructions are promoted into backend instructions.                                                                                                                                                                                               | Keep enabled so OpenAI uses the provided skill body instead of looking for local skill files.                                                 |
|    |                                               | `true`           | Enables skill translation. The selected mode decides how the skill block is translated.                                                                                                                                                                                                                   | Default; normal gateway behavior.                                                                                                             |
|    |                                               | `false`          | Disables skill translation and forwards skill tags as ordinary text.                                                                                                                                                                                                                                      | Use only for debugging raw request translation.                                                                                               |
| 4  | `workflow.claude_code.skills.mode`            | Purpose          | Selects which expanded skill block is active and how it is translated.                                                                                                                                                                                                                                    | This only matters when `skills.enabled: true`; slash-command translation takes precedence when both are present.                              |
|    |                                               | `promote_active` | Only supported value. If the active/latest user message contains expanded skill instructions, Gateway moves the skill body into backend `instructions`, sends skill args as user input, and tells the backend not to search/load local `~/.claude` skill files.                                           | Use this for Claude Code skill-expanded requests.                                                                                             |
|    |                                               | any other value  | Config parse fails because the enum value is unsupported.                                                                                                                                                                                                                                                 | Do not set another mode until the gateway implements and documents it.                                                                        |
...
|    |                                        | unset                               | Gateway falls back to `GATEWAY_HOME/config.yml`, then `~/.gateway/config-dev.yml`.                                               | Normal for local development.                                                                       |
...
|    |                                        | `/tmp/gateway-dev`                  | Uses `/tmp/gateway-dev/config.yml`, `/tmp/gateway-dev/auth.json`, logs, and state paths unless more specific vars override them. | Useful for isolated dev/test runs.                                                                  |
