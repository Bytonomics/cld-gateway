...
class TestPackageMetadataValidation:
    ...
    def test_archive_contains_binary_metadata_and_package_assets(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert "commands/gateway/plugin/commands/status.md" in names, "Gateway status asset missing from archive"
            assert "commands/gateway/.claude-plugin/marketplace.json" in names, "Gateway plugin marketplace manifest missing from archive"
            assert "commands/gateway/plugin/.claude-plugin/plugin.json" in names, "Gateway plugin manifest missing from archive"
