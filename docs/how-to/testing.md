...
|---------|----------------|
| [The CONTRACT / FAKE / SLOP-ADJACENT labeling](#the-contract--fake--slop-adjacent-labeling) | How a Rust test earns a place in the Go port |
| [Migration matrix](#migration-matrix) | Where the full behavior-to-test mapping lives |
| [Seam choice: the HTTP boundary](#seam-choice-the-http-boundary) | The primary test seam for behavior-level tests |
| [Flush-invariant stream tests](#flush-invariant-stream-tests) | Asserting per-event delivery, not just the final buffer |
| [Golden files](#golden-files) | When to compare against a golden file instead of an inline value |
...
- **SLOP-ADJACENT** — depends on the prompt-text heuristics documented in
  `docs/AI_SLOP.md`. Do not port these as-is; the underlying behavior they test
  is either being replaced with a deterministic check (see
  [Design decisions](../explanation/design-decisions.md)) or, for the
...
app → HTTP API → CLI) live in `docs/test-audit-and-migration-plan.md`
...
Because the single-writer SSE model ([ADR-0004](../decisions/ADR-0004-sse-single-writer-goroutine.md)) exists
