...
1. `crates/gateway-http-anthropic/src/claude_code_inclusion.rs`
2. `crates/gateway-http-anthropic/src/claude_code_context.rs`
3. `crates/gateway-http-anthropic/src/translate.rs`
...
1. `local-only`
   ...
   - Implementation target:
     - `LOCAL_ONLY_COMMANDS` in `crates/gateway-http-anthropic/src/claude_code_inclusion.rs`.
   - Required data:
     - normalized command name without leading `/`
     - stdout marker text that uniquely identifies the local CLI result, when Claude Code emits stdout
...
2. `read-only`
   ...
   - Implementation target:
     - read-only detection in `crates/gateway-http-anthropic/src/claude_code_inclusion.rs`.
   - Required data:
     - stable marker text that Claude Code emits for this kind of side turn
   - Behavior to preserve:
     - keep the user’s side question visible to the model
     - set `gateway_conversation_inclusion` to `read_only`
     - do not classify a normal message containing the literal command name as read-only
...
1. Inspect `~/.gateway/logs/http-exchange.jsonl`.
...
2. Reuse the existing conversation inclusion layer. Do not add ad-hoc filtering in translation call sites.
3. Keep command matching normalized so `/name` and `name` resolve consistently where the existing code supports it.
4. Add or update tests in `crates/gateway-http-anthropic/src/translate.rs`.
...
2. If formatting fails only because of your edits, run `cargo fmt`, then rerun `cargo fmt --check`.
3. Run the most focused relevant test first.
4. Run `cargo clippy -p gateway-http-anthropic --all-targets --all-features -- -D warnings`.
5. If the user asks for full validation, run `make test`.
