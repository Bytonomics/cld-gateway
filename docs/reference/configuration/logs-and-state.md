...
|---------|----------------|
| [Credentials — `auth.json`](#credentials--authjson) | Stored backend tokens |
| [Exchange log — `logs/http-exchange.log`](#exchange-log--logshttp-exchangelog) | Per-request/response entries |
| [Transport decisions — `logs/transport-decisions.jsonl`](#transport-decisions--logstransport-decisionsjsonl) | Per-turn transport choice log |
| [Conversation state](#conversation-state--gatewaysessionsclaudecode) | Durable per-conversation memory |
...
[Troubleshooting](../../runbooks/troubleshooting.md) for how to use this.
...
## Conversation state — `~/.gateway/sessions/claudecode/`
